...
function commandNeedsSelector(attr) {
...
export async function main(argv = process.argv.slice(2)) {
  ...
  if (command === 'select') {
    ...
    const payload = await requestCommand(flags.server, 'select', { selector, value });
    writeResult(payload);
    return 0;
  }
  ...
  if (command === 'drag') {
    ...
    const payload = await requestCommand(flags.server, 'drag', { start, end: end || '' });
    writeResult(payload);
    return 0;
  }
  ...
  if (command === 'upload') {
    ...
    const payload = await requestCommand(flags.server, 'upload', { selector, files });
    writeResult(payload);
    return 0;
  }
  ...
  if (command === 'window') {
    const action = rest[0];
    if (action === 'new') {
      const payload = await requestCommand(flags.server, 'window', { action: 'new' });
      writeResult(payload);
      return 0;
    }
  ...
  if (command === 'is') {
    ...
    const payload = await requestCommand(flags.server, 'is', { selector, state });
    writeResult(payload);
    return 0;
  }
  ...
  if (command === 'get') {
    ...
    const payload = await requestCommand(flags.server, 'get', { selector, attr });
    writeResult(payload);
    return 0;
  }
  ...
  if (command === 'dialog') {
    ...
    const payload = await requestCommand(flags.server, 'dialog', { accept, promptText });
    writeResult(payload);
    return 0;
  }
  ...
  if (command === 'wait') {
    ...
    let waitArgs = { timeout };

    if (type === 'time' || type === 'ms') {
      waitArgs = { type: 'time', ms: Number(value) || timeout };
    } else if (type === 'selector') {
      waitArgs = { type: 'selector', selector: value, timeout };
    } else if (type === 'url') {
      waitArgs = { type: 'url', url: value, timeout };
    } else if (type === 'text') {
      waitArgs = { type: 'text', text: value, timeout };
    } else if (type === 'load') {
      waitArgs = { type: 'load', timeout };
    } else if (type === 'networkidle') {
      waitArgs = { type: 'networkidle', timeout };
    } else if (!isNaN(Number(type))) {
      // 纯数字表示等待毫秒数
      waitArgs = { type: 'time', ms: Number(type) };
    } else {
  ...
  if (command === 'cookies') {
    const action = rest[0];
    if (action === 'get') {
      const payload = await requestCommand(flags.server, 'cookies', { action: 'get' });
      writeResult(payload);
      return 0;
    }
    if (action === 'set') {
    ...
    if (action === 'clear') {
      const payload = await requestCommand(flags.server, 'cookies', { action: 'clear' });
      writeResult(payload);
      return 0;
    }
  ...
  if (command === 'storage') {
    ...
    if (action === 'get') {
      const key = rest[1];
      const payload = await requestCommand(flags.server, 'storage', { action: 'get', key });
      writeResult(payload);
      return 0;
    }
    if (action === 'set') {
      ...
      const payload = await requestCommand(flags.server, 'storage', { action: 'set', key, value });
      writeResult(payload);
      return 0;
    }
    if (action === 'clear') {
      const payload = await requestCommand(flags.server, 'storage', { action: 'clear' });
      writeResult(payload);
      return 0;
    }
  ...
  if (command === 'set') {
    ...
    if (type === 'offline') {
      ...
      const payload = await requestCommand(flags.server, 'set', { type: 'offline', enabled });
      writeResult(payload);
      return 0;
    }
    if (type === 'headers') {
      // headers 是 JSON 格式: key:value,key2:value2
      const headers = subArgs
        ...
        .filter((h) => h.name);
      const payload = await requestCommand(flags.server, 'set', { type: 'headers', headers });
      writeResult(payload);
      return 0;
    }
    if (type === 'geo') {
    ...
    if (type === 'media') {
      ...
      const payload = await requestCommand(flags.server, 'set', { type: 'media', media });
      writeResult(payload);
      return 0;
    }
  ...
  if (command === 'clipboard') {
    const action = rest[0];
    if (action === 'read') {
      const payload = await requestCommand(flags.server, 'clipboard', { action: 'read' });
      writeResult(payload);
      return 0;
    }
    if (action === 'write') {
      const text = rest.slice(1).join(' ');
      const payload = await requestCommand(flags.server, 'clipboard', { action: 'write', text });
      writeResult(payload);
      return 0;
    }
  ...
  if (command === 'state') {
    ...
    if (action === 'save') {
      ...
      const payload = await requestCommand(flags.server, 'state', { action: 'save', name });
      writeResult(payload);
      return 0;
    }
    if (action === 'load') {
